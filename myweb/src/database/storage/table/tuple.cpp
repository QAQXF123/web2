...
#include "../../a-include/storage/table/tuple.h"
...
Tuple::Tuple(std::vector<Value> values, const Schema *schema) : allocated_(true) {
  //printf("values.size():%d  schema->GetColumnCount():%d\n", values.size(), schema->cnt());
  assert(values.size() == schema->cnt());

  // 1. Calculate the size of the tuple.
  uint32_t tuple_size = schema->GetLength();
  //printf("schema len tuple size :%d\n", tuple_size);
